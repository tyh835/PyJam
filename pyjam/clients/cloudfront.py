...
class CloudFrontClient:
    ...
    def find_matching_cert(self, domain_name):
        ...
        for page in paginator.paginate(CertificateStatuses=['ISSUED']):
            for cert in page['CertificateSummaryList']:
                if self.certificate_matches(cert['CertificateArn'],
                                            domain_name):
                    return cert
    ...
    def create_distribution(self, bucket_name):
        ...
        try:
            ...
            origin_id = 'S3-Website-' + '{0}.{1}'.format(
                bucket_name,
            ...
            if not certificate:
                ...
                print(
                    'Please request a public certificate from the AWS Certificate Manager'
                )
                print('Add both your <domain> and *.<domain>')
                print('and make sure to use us-east-1 (N. Virginia)!\n')
                return

            print('Creating CloudFront distribution for {0}...'.format(
                bucket_name))
            ...
            print('Unable to create distribution for {0}. '.format(bucket_name)
                  + str(err) + '\n')
    ...
    def await_deploy(self, distribution):
        ...
        print('\nDomain configured: https://{0}'.format(
