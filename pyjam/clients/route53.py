...
class Route53Client:
    ...
    def find_matching_bucket(self, domain_name):
        ...
        try:
            ...
            print('Unable to find bucket {0}. '.format(domain_name) +
                  str(err) + '\n')
            print('Please run `jam setup bucket {0}` first. '.format(
                domain_name))
    ...
    def create_s3_domain_record(self, domain_name):
        ...
        try:
            ...
            print('\nUnable to create Alias record to point to {0}. '.format(
                domain_name + '.' + endpoint.host) + str(err) + '\n')
    ...
    def create_cf_domain_record(self, domain_name):
        ...
        try:
            zone = find_hosted_zone(self.route53, domain_name) \
            or create_hosted_zone(self.route53, domain_name)
            cf_domain = self.find_matching_distribution(domain_name).get(
                'DomainName', None)
            ...
            print('Unable to create Alias record for {0}. '.format(domain_name)
                  + str(err) + '\n')
